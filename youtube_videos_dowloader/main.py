        ...
        if self.cliente_type_choice == 'video':
            while True:
                try:
                    ...
                    # Para 360p
                    if self.cliente_quality_choice == '1':

                        # Get Video:
                        if self.link_video.streams.filter(
                                resolution="360p", mime_type='video/webm'):
                            self.video_file = self.link_video.streams.filter(
                                resolution="360p", mime_type='video/webm')
                        elif self.link_video.streams.filter(
                                resolution="360p", mime_type='video/mp4'):
                            self.video_file = self.link_video.streams.filter(
                                resolution="360p", mime_type='video/mp4'
                            )
                        else:
                            raise ValueError(
                                "Infelizmente, essa qualidade para"
                    ...
                    # Para 480p
                    elif self.cliente_quality_choice == '2':

                        # Get Video:
                        if self.link_video.streams.filter(
                                resolution="480p", mime_type='video/webm'):
                            self.video_file = self.link_video.streams.filter(
                                resolution="480p", mime_type='video/webm')
                        elif self.link_video.streams.filter(
                                resolution="480p", mime_type='video/mp4'):
                            self.video_file = self.link_video.streams.filter(
                                resolution="480p", mime_type='video/mp4'
                            )
                        else:
                    ...
                    # Para 720p
                    elif self.cliente_quality_choice == '3':

                        # Get Video:
                        if self.link_video.streams.filter(
                                resolution="720p", mime_type='video/webm'):
                            self.video_file = self.link_video.streams.filter(
                                resolution="720p", mime_type='video/webm')
                        elif self.link_video.streams.filter(
                    ...
                    # Para 1080p
                    elif self.cliente_quality_choice == '4':

                        # Get Video:
                        if self.link_video.streams.filter(
                                resolution="1080p", mime_type='video/webm'):
                            self.video_file = self.link_video.streams.filter(
                                resolution="1080p", mime_type='video/webm')
                        elif self.link_video.streams.filter(
                                resolution="1080p", mime_type='video/mp4'):
                            self.video_file = self.link_video.streams.filter(
                                resolution="1080p", mime_type='video/mp4'
                            )
                        else:
                    ...
                    print(self.cliente_quality_choice)
                    return self.cliente_quality_choice
                except ValueError as err:
                    print(err)
